...
func TestBuilder(t *testing.T) {
	...
	items := []struct {
		...
		{
			types.NodePath{"a"}, 0,
			yaml.NewMapRNode(nil).YNode(),
		},
		{
			types.NodePath{"a", "b"}, 0,
			yaml.NewStringRNode("v1").YNode(),
		},
		{
			types.NodePath{"x", "y", "z"},
			yaml.SequenceNode,
			nil,
		},
		{
			types.NodePath{"x", "y", "z", "[name=u]"},
			yaml.MappingNode,
			nil,
		},
		{
			types.NodePath{"x", "y", "z", "[name=u]", "v"},
			0,
			yaml.NewStringRNode("v2").YNode(),
		},
		{
			types.NodePath{"x", "y", "z", "[name=w]"},
			yaml.MappingNode,
			nil,
		},
		{
			types.NodePath{"x", "y", "z", "[name=w]", "v"},
			0,
			yaml.NewStringRNode("v3").YNode(),
		},
		{
			types.NodePath{"x", "l"},
			0,
